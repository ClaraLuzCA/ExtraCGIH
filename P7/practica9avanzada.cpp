...
int main()
{
	...
	while (!mainWindow.getShouldClose())
	{
		...
		toffsetflechav += 0.0;//PARA QUE SE MUEVA NE DIAGONAL
		//para que no se desborde la variable
		if (toffsetflechau > 1.0)//PARA QUE NO SE DESBORDE LA VARIABLE
			toffsetflechau = 0.0;
		//if (toffsetv > 1.0)
		//	toffsetv = 0;
		//printf("\ntfosset %f \n", toffsetu);
		//pasar a la variable uniform el valor actualizado
		toffset = glm::vec2(toffsetflechau, toffsetflechav);//SE MANDAN LAS DOS VARIABLE

		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(-2.0f, 1.0f, -6.0f));
		model = glm::rotate(model, 90 * toRadians, glm::vec3(1.0f, 0.0f, 0.0f));
		model = glm::scale(model, glm::vec3(3.0f, 3.0f, 3.0f));
		glUniform2fv(uniformTextureOffset, 1, glm::value_ptr(toffset));//SE MANDA AL SHADER
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		...
		for (int j = 1; j < 5; j++)
		{
			//números 5-8
			toffsetnumerou += 0.25;
			toffsetnumerov = -0.33;//CORRDENADA PARA BAJARA A LA SEGUNDA FILA
			toffset = glm::vec2(toffsetnumerou, toffsetnumerov);
			model = glm::mat4(1.0);
		...
		*/
		...
		cambiar1_2 =cambiar1_2+ 0.01*deltaTime;
